...
def validate_llm_answer(answer, results):
    ...
    # Ignore tiny numbers such as page references.
    meaningful_answer_numbers = {
        number
        for number in answer_numbers
        if len(number.replace(",", "").split(".")[0]) >= 3
    }

    if not meaningful_answer_numbers:

        return True

    return bool(
        meaningful_answer_numbers.intersection(
            evidence_numbers
        )
    )
...
def main():
    ...
    while True:
        ...
        financial_answer = None

        query_lower = query.lower()

        if (
            "revenue" in query_lower
            and "consolidated" in query_lower
        ):

            financial_answer = (
                extract_consolidated_revenue(
                    results
                )
            )

        # ----------------------------------------------------
        # IF EXACT FINANCIAL VALUE FOUND
        # ----------------------------------------------------
        ...
        if financial_answer:
            ...
            final_answer = format_answer(
                ...
                results,
                forced_page=financial_answer["page"]
            )
